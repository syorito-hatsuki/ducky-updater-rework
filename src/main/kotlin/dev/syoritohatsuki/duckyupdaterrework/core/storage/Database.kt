...
object Database {
    ...
    private val DB_PATH: String = "${FabricLoader.getInstance().configDir.toAbsolutePath()}/durw/cache.db"
    ...
    fun insertOrUpdateProject(
        ...
        when (loader) {
            Loader.FABRIC -> {
                ...
                update(
                    when {
                        isProjectExist(projectId, loader = Loader.FABRIC) -> """UPDATE projects 
            ...
            Loader.DATAPACK -> {
                ...
                update(
                    when {
                        isProjectExist(projectId, loader = Loader.DATAPACK) -> """UPDATE datapacks 
                            SET ${updateValues.entries.joinToString(",") { "'${it.key}' = '${it.value}'" }} 
                            WHERE projectId = '$projectId'
                        """.trimIndent()

                        else -> """INSERT INTO datapacks (
                                ${updateValues.keys.joinToString(",")}) 
                            VALUES (
                                ${updateValues.values.joinToString(",") { "'$it'" }}
                        )""".trimIndent()
    ...
    fun markProjectAsUpdated(projectId: ProjectId, loader: Loader) {
        when (loader) {
            Loader.FABRIC -> update("UPDATE projects SET outdated = 0 WHERE projectId = '$projectId'")
            Loader.DATAPACK -> update("UPDATE datapacks SET outdated = 0 WHERE projectId = '$projectId'")
        }
    }
    ...
    private fun isProjectExist(projectId: ProjectId? = null, modId: ModId? = null, loader: Loader): Boolean {
        ...
        when (loader) {
            ...
            Loader.DATAPACK -> query(
                when {
                    projectId?.isNotBlank() == true -> "SELECT projectId FROM datapacks WHERE projectId = '${projectId}' LIMIT 1"
                    else -> return false
                }
            ) {
                projectExist = it.next()
    ...
    fun getOutdatedProjectIds(loader: Loader): ArrayListMultimap<ProjectId, DependencyId> {
        ...
        if (loader == Loader.DATAPACK) {
            query("SELECT projectId FROM datapacks WHERE ignore = FALSE AND outdated = TRUE") {
                while (it.next()) {
                    DuckyUpdaterReWork.logger.warn("Found: ${it.type} | ${it.row}")
                    modsIds.put(it.getString("projectId"), "")
                }
            }
        }
    ...
    fun getAdditionalInfoByProjectIds(
        projectIdsMultimap: ArrayListMultimap<ProjectId, DependencyId>, loader: Loader
    ): MutableMap<ProjectId, AdditionalInfo> {
        ...
        when (loader) {
            ...
            Loader.DATAPACK -> query(
                """SELECT projectId, name, changelog, url, version 
                    FROM datapacks 
                    WHERE projectId IN(${projectIds.joinToString(prefix = "'", postfix = "'", separator = "','")}) 
                    LIMIT ${projectIds.size}
                """.trimIndent()
                ...
                while (it.next()) additionalInfos[it.getString("projectId")] = AdditionalInfo(
                    ...
                    changeLog = it.getString("changelog") ?: "",
                    url = it.getString("url") ?: "",
                    version = Version(currentVersion = "", newVersion = it.getString("version") ?: "")
                )
            }
        }
    ...
    fun getAllDownloadingData(loader: Loader) = mutableMapOf<ProjectId, Pair<Url, Filename>>().apply {
        when (loader) {
            Loader.FABRIC -> query(
            ...
            Loader.DATAPACK -> query(
                """SELECT datapacks.projectId, datapacks.filename, datapacks.url 
                FROM datapacks
                WHERE ignore = FALSE
                AND outdated = TRUE
                """.trimIndent()
    ...
    fun getDownloadingDataByProjectIds(projectIds: Set<ProjectId>, loader: Loader) =
        mutableMapOf<ProjectId, Pair<Url, Filename>>().apply {
            when (loader) {
                ...
                """.trimIndent()
                    ...
                    while (it.next()) put(
                        it.getString("projectId"), Pair(
                            it.getString("url") ?: continue,
                            it.getString("filename") ?: DuckyUpdaterApi.defaultDatapacksDir?.absolutePathString()
                            ?: return@query
                        )
                    )
                }
    ...
    fun getDirectoriesByProjectId(projectIds: Set<ProjectId>, loader: Loader) =
        mutableMapOf<ProjectId, FilePath>().apply {
            when (loader) {
                Loader.FABRIC -> query(
                ...
                Loader.DATAPACK -> query(
                    """SELECT datapacks.projectId, datapacks.filePath 
                        FROM datapacks
                        WHERE datapacks.projectId IN(${
                        projectIds.joinToString(
                            prefix = "'", postfix = "'", separator = "','"
                        )
                    })
                    ...
                    while (it.next()) put(
                        it.getString("projectId"),
                        it.getString("filePath") ?: DuckyUpdaterApi.defaultDatapacksDir?.absolutePathString()
                        ?: return@query
                    )
                }
            }
    ...
    fun getListOfIgnoredProjects(loader: Loader) = mutableMapOf<String, String>().apply {
        when (loader) {
            Loader.FABRIC -> query(
            ...
            Loader.DATAPACK -> query(
                """SELECT datapacks.name, datapacks.filename
                    FROM datapacks 
                    WHERE datapacks.ignore = TRUE
                """.trimIndent()
            ) {
                while (it.next()) put(it.getString("name"), it.getString("filename").substringBeforeLast("."))
            }
        }
    }
